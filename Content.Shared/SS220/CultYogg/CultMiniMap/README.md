...
обновляется раз в секунду. Снимок хранится в `CultMiniMapComponent.State` и
передаётся только клиенту, управляющему владельцем. В общее состояние UI снимок
не записывается; клиент обновляет окно после получения состояния компонента.

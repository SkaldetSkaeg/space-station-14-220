...
public sealed partial class ServerGameTicker
{
    ...
    /// </summary>
    public Entity<GameRuleComponent>? AddGameRule(EntProtoId ruleId, GameRuleSource? source)
    {
        var rule = AddGameRule(ruleId);
        if (rule != null)
            _eventHistory.RecordSource(rule.Value, source);
    ...
    public void ClearGameRules(string? endedBy = null)
    {
        foreach (var rule in GetAddedGameRules())
        {
            EndGameRule(rule, GameRuleEndReason.RulesCleared, endedBy);
        }
    }
    ...
    private void AddGameRuleCommand(IConsoleShell shell, string argstr, string[] args)
    {
        ...
        foreach (var rule in args)
        {
            ...
            _eventHistory.RecordSource(ent, new GameRuleSource(
                shell.Player == null ? GameRuleSourceKind.ServerConsole : GameRuleSourceKind.Administrator,
                shell.Player?.Name));
    ...
    private void EndGameRuleCommand(IConsoleShell shell, string argstr, string[] args)
    {
        ...
        foreach (var rule in args)
        {
            ...
            EndGameRule(ruleEnt.Value, shell.Player == null ? GameRuleEndReason.ServerConsole : GameRuleEndReason.Administrator, shell.Player?.Name);
        }
    }

...
public sealed partial class EventManagerSystem : EntitySystem
{
    ...
    public void RunRandomEvent(EntityTableSelector limitedEventsTable, EntityUid? scheduler = null)
    {
        ...
        _gameTicker.AddGameRule(randomLimitedEvent, source);
    }
    ...
    private bool CanRun(EntityPrototype prototype, StationEventComponent stationEvent, int playerCount, TimeSpan currentTime)
    {
        ...
        // Ended rules lose their component, so retain round limits using the recorded starts.
        var (count, lastRun, active) = _history.GetStartStatistics(prototype.ID);
        if (active)
            return false;
